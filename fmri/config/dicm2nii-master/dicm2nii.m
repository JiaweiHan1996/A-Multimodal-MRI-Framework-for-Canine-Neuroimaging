...
%% user preference
pf.save_patientName = getpref('dicm2nii_gui_para', 'save_patientName', false);
...
%         try dwell = s.AcquisitionDuration; % not confirmed yet
        if isfield(s, 'Private_0019_1028') && ~isempty(s.Private_0019_1028) && ...
			isnumeric(s.Private_0019_1028) && isscalar(s.Private_0019_1028)
             dwell = s.Private_0019_1028;
             dwell = 1 / dwell / (dim(iPhase)); 
        elseif isfield(s, 'EffectiveEchoTime') && ~isempty(s.EffectiveEchoTime) && ...
				isnumeric(s.EffectiveEchoTime) && isscalar(s.EffectiveEchoTime)
               dwell = s.EffectiveEchoTime / 1000;
        else
                dwell = 0;
        end
    end
    ...
    if ~isempty(dwell)
        s.EffectiveEPIEchoSpacing = dwell * 1000;
        % https://github.com/rordenlab/dcm2niix/issues/130
        readout = dwell * (dim(iPhase)- 1) ; % since 170923
        s.ReadoutSeconds = readout;
        s.ReconMatrixPE = dim(iPhase);
...
flds = { % store for nii.ext and json
  'Manufacturer' 'MagneticFieldStrength' 'SeriesDescription' 'ImageType' 'Modality' ...
  'AcquisitionDateTime' 'TaskName' 'bval' 'bvec' 'VolumeTiming' ...
  'ReadoutSeconds' 'DelayTimeInTR' 'RepetitionTime' ...
  ...
  'UnwarpDirection' 'EffectiveEPIEchoSpacing'  'ReconMatrixPE' 'EchoTime' 'deltaTE' 'EchoTimes' ...
  ...
  'TableHeight' 'DistanceSourceToPatient' 'DistanceSourceToDetector' 'SliceTiming' 'ConversionSoftware'};
