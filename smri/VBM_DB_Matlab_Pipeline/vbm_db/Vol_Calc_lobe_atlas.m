function Vol_Calc_lobe_atlas(opDir, ipDir, Sub_ID, TMP)
% Calculate Volumes of WholeBrain & ROIs
% - opDir: excel output pathway, eg. '...\Results'
% - ipDir: data input pathway, eg. '...\Results\Volumes'
% ========== Initialization ========== %
out_fname = 'Volumes_results_LOBE_atlas';
xls_path = [opDir filesep out_fname '.xlsx'];

% ========= Lobe Atlas ==========
Atlas_Info = read_atlas_txt(TMP.atlas_lobe_info);
% -----
Atlas_path = TMP.atlas_lobe;
...
for i = 1:length(Sub_ID)
    ...
    vox = [...
        sqrt(sum(affine(1:3,1).^2)), ... % x dir
        sqrt(sum(affine(1:3,2).^2)), ... % y dir
        sqrt(sum(affine(1:3,3).^2))  ... % z dir
        ];
