function mask=rp_loadmask(AX, AY, AZ, AMaskFilename)
...
elseif( strcmpi(AMaskFilename, 'Default')||( isnumeric(AMaskFilename) && AMaskFilename==1) ) % like the old parameter	, back-compatible  Xiaowei Song, 20070421
	switch int2str([AX, AY, AZ])
	    case '79  95  69'   % 'default''[2 2 2]'
	        [mask, vsizeTmp, Header]=rp_readfile([pathstr '/mask/BrainMask_05_79x95x69.img']); %YAN Chao-Gan 081225: New masks.
	    case '53  63  46'   % 'default''[3 3 3]'	        
			[mask, vsizeTmp, Header]=rp_readfile([pathstr '/mask/BrainMask_05_53x63x46.img']); %YAN Chao-Gan 081225: New masks.
	    case '91  109   91'  % 'template''[2 2 2]'	        
			[mask, vsizeTmp, Header]=rp_readfile([pathstr '/mask/BrainMask_05_91x109x91.img']); %YAN Chao-Gan 081225: New masks.
	    case '61  73  61'   % 'template' '[3 3 3]'	        
			[mask, vsizeTmp, Header]=rp_readfile([pathstr '/mask/BrainMask_05_61x73x61.img']); %YAN Chao-Gan 081225: New masks.
	    otherwise
	...
	if (ischar(AMaskFilename))
		...
		if strcmpi(AMaskFilename(end-3:end), '.img') || strcmpi(AMaskFilename(end-3:end), '.nii')
			[mask, vsizeTmp, Header]=rp_readfile(AMaskFilename);            
